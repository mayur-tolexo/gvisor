...
// Binary fuse_host implements a FUSE protocol server that runs on the host
// and communicates over a socketpair. One end of the socketpair is intended
// to be passed into a gVisor sandbox as a host FD, where it is used to mount
// a FUSE filesystem via the host passthrough path.
//
// The server implements a loopback filesystem: it forwards all operations
// to a backing directory on the host.
//
// Usage:
//
//	fuse_host --back-dir=/path/to/backing --fd=N
//
// where N is the server-side FD of the socketpair (the caller creates the
// socketpair and passes the server end as an extra file).
package main
...
var (
	backDir = flag.String("back-dir", "", "The backing directory on the host for the loopback FUSE server.")
	serverFD = flag.Int("fd", -1, "The server-side socketpair FD to read FUSE requests from.")
)

func main() {
	flag.Parse()
	if *backDir == "" {
		log.Warningf("fuse_host: --back-dir is required")
		os.Exit(1)
	}
	if *serverFD < 0 {
		log.Warningf("fuse_host: --fd is required")
		os.Exit(1)
	}
...
func (s *server) serve() {
	for {
		...
		if n < int(linux.SizeOfFUSEHeaderIn) {
			log.Warningf("fuse_host: short request %d bytes", n)
			return
		}
		buf = buf[:n]
		...
		if _, err := unix.Write(s.fd, resp); err != nil {
			log.Warningf("fuse_host: write error: %v", err)
			return
		}
	}
}
...
func (s *server) handleRequest(hdr *linux.FUSEHeaderIn, payload []byte) []byte {
	...
	case linux.FUSE_GETATTR:
		return s.handleGetAttr(hdr, payload)
...
func (s *server) handleGetAttr(hdr *linux.FUSEHeaderIn, payload []byte) []byte {
...
func (s *server) handleLookup(hdr *linux.FUSEHeaderIn, payload []byte) []byte {
	...
	// Use inode number from stat as the node ID.
...
// nodeIDToPath converts a FUSE node ID to a host filesystem path.
// Node ID 1 (FUSE_ROOT_ID) maps to the backing directory.
// Other node IDs are host inode numbers; we search the backing directory
// for a matching inode. For simplicity, this only supports a flat directory.
