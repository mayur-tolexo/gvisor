...
// If a task is not visible in that namespace or is a zombie (or worse), its TID is
// not included in the returned map. If threadGroupIDs is true, the returned
// thread IDs correspond to the thread groups of the given tasks.
func (ns *PIDNamespace) IDsOfTasks(tasks []*Task, threadGroupIDs bool) map[ThreadID]struct{} {
	...
	for _, task := range tasks {
		if task.exitStateLocked() < TaskExitZombie {
			var id ThreadID
			if threadGroupIDs {
				id = ns.tgids[task.tg]
			} else {
