...
func (fs *filesystem) newRootInode(ctx context.Context, mode linux.FileMode) kernfs.Inode {
	...
	c.IncLinks(c.OrderedChildren.Populate(fs.rootInterfaceFiles(ctx, fs.rootCreds.EffectiveKUID, fs.rootCreds.EffectiveKGID, c)))
...
func (fs *filesystem) newCgroupLocked(ctx context.Context, creds *auth.Credentials, mode linux.FileMode, path string, parent *cgroup) kernfs.Inode {
	...
	c.IncLinks(c.OrderedChildren.Populate(fs.cgroupInterfaceFiles(ctx, creds.EffectiveKUID, creds.EffectiveKGID, c)))
