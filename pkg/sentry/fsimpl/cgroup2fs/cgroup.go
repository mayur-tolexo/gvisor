...
func (c *cgroup) CanEnter(ctx context.Context, t *kernel.Task) bool {
	curSet := c.closestCtrls.Load()
	if curSet == nil {
		return true
	}
...
// CancelEnter cancels an Enter().
func (c *cgroup) CancelEnter(ctx context.Context, t *kernel.Task) {
	curSet := c.closestCtrls.Load()
	if curSet == nil {
		return
	}
...
// Enter is called when during clone() or CreateProcess().
func (c *cgroup) Enter(ctx context.Context, t *kernel.Task) {
	c.fs.tasksMu.Lock()
	c.tasks[t] = struct{}{}
...
func (c *cgroup) populateInterfaceFiles(ctx context.Context, ctrl controller) {
	...
	for _, def := range ctrl.interfaceFiles() {
		...
		inode := c.fs.newInterfaceFile(ctx, uid, gid, c, def)
		c.OrderedChildren.Insert(def.name, inode)
	}
}
...
func (c *cgroup) kill() {
	...
	// No need to continue to hold treeMu while delivering
	// the signals, hence the defer.
	defer func() {
