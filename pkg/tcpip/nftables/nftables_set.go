...
func exprInfoToSetOp(exprInfo *ExprInfo) (operation, *syserr.AnnotatedError) {
	if exprInfo == nil {
		return nil, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Expression information is nil")
	}
	opType := ToOpType(exprInfo.ExprName)
	if opType == OpTypeUnknown {
		return nil, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Invalid set expression name")
	}
	...
	case OpTypeCounter:
		...
		return op, nil
	default:
		return nil, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Unsupported set expression type")
	}
}
...
func validateSetFlags(flags uint16) *syserr.AnnotatedError {
	if flags&^(linux.NFT_SET_ANONYMOUS|linux.NFT_SET_CONSTANT|
		...
		return syserr.NewAnnotatedError(syserr.ErrNotSupported, "Nftables: Unsupported set flags")
	}
	if (flags & (linux.NFT_SET_MAP | linux.NFT_SET_OBJECT)) ==
		(linux.NFT_SET_MAP | linux.NFT_SET_OBJECT) {
		return syserr.NewAnnotatedError(syserr.ErrNotSupported, "Nftables: Only one of NFT_SET_MAP or NFT_SET_OBJECT is supported")
	}
	if (flags & (linux.NFT_SET_EVAL | linux.NFT_SET_OBJECT)) ==
		(linux.NFT_SET_EVAL | linux.NFT_SET_OBJECT) {
		return syserr.NewAnnotatedError(syserr.ErrNotSupported, "Nftables: Only one of NFT_SET_EVAL or NFT_SET_OBJECT is supported")
	}
	if (flags & (linux.NFT_SET_ANONYMOUS | linux.NFT_SET_TIMEOUT | linux.NFT_SET_EVAL)) ==
		(linux.NFT_SET_ANONYMOUS | linux.NFT_SET_TIMEOUT) {
		return syserr.NewAnnotatedError(syserr.ErrNotSupported, "Nftables: Only one of NFT_SET_ANONYMOUS or NFT_SET_TIMEOUT is supported")
	}
	if (flags & (linux.NFT_SET_CONSTANT | linux.NFT_SET_TIMEOUT)) ==
		(linux.NFT_SET_CONSTANT | linux.NFT_SET_TIMEOUT) {
		return syserr.NewAnnotatedError(syserr.ErrNotSupported, "Nftables: Only one of NFT_SET_CONSTANT or NFT_SET_TIMEOUT is supported")
	}
	return nil
}
...
func parseSetDataAttr(attrs map[uint16]nlmsg.BytesView, setFlags uint16) (dataType uint32, dataLen uint32, err *syserr.AnnotatedError) {
	...
	if !ok {
		if setFlags&linux.NFT_SET_MAP != 0 {
			return 0, 0, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Set data type attribute is missing")
		}
		return 0, 0, nil
	}
	if setFlags&linux.NFT_SET_MAP == 0 {
		return 0, 0, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Set data type attribute should not be present for non-map set")
	}
	if (dataType&linux.NFT_DATA_RESERVED_MASK) == linux.NFT_DATA_RESERVED_MASK &&
		dataType != linux.NFT_DATA_VERDICT {
		return 0, 0, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Unsupported set data type")
	}
	...
	if !ok {
		return 0, 0, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Set data length attribute is missing")
	}
	if dataLen == 0 || dataLen > linux.NFT_DATA_VALUE_MAXLEN {
		return 0, 0, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Invalid set data length")
	}
...
func parseSetObjectAttr(attrs map[uint16]nlmsg.BytesView, setFlags uint16) (objType uint32, err *syserr.AnnotatedError) {
	...
	if ok && (setFlags&linux.NFT_SET_OBJECT == 0) {
		return 0, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Set object flag is not set but object type attribute is present")
	}
	if !ok && (setFlags&linux.NFT_SET_OBJECT != 0) {
		return 0, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Set object flag is set but object type attribute is missing")
	}
	if !ok {
		return 0, nil
	}
	if objType == linux.NFT_OBJECT_UNSPEC || objType > linux.NFT_OBJECT_MAX {
		return 0, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Invalid set object type")
	}
	return objType, nil
}
...
func parseSetTimeoutAttr(attrs map[uint16]nlmsg.BytesView, setFlags uint16) (timeMiliSec uint64, err *syserr.AnnotatedError) {
	...
	if ok && (setFlags&linux.NFT_SET_TIMEOUT == 0) {
		return 0, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Set timeout flag is not set but timeout attribute is present")
	}
	if !ok {
		return 0, nil
	}
	if setFlags&linux.NFT_SET_ANONYMOUS != 0 {
		return 0, syserr.NewAnnotatedError(syserr.ErrNotSupported, "Nftables: Set timeout flag is not supported for anonymous sets")
	}
	return timeMS, nil
}
...
func parseSetGCIntervalAttr(attrs map[uint16]nlmsg.BytesView, setFlags uint16) (gcIntMiliSec uint32, err *syserr.AnnotatedError) {
	...
	if setFlags&linux.NFT_SET_TIMEOUT == 0 {
		return 0, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Set gc flag is not set but gc attribute is present")
	}
	if setFlags&linux.NFT_SET_ANONYMOUS != 0 {
		return 0, syserr.NewAnnotatedError(syserr.ErrNotSupported, "Nftables: Set gc flag is not supported for anonymous sets")
	}
	return gc, nil
}
...
func parseSetPolicyAttr(attrs map[uint16]nlmsg.BytesView, setFlags uint16) (policy uint32, err *syserr.AnnotatedError) {
	...
	if policy != linux.NFT_SET_POL_PERFORMANCE && policy != linux.NFT_SET_POL_MEMORY {
		return 0, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Invalid set policy")
	}
	return policy, nil
}
...
func parseSetConcatAttr(descConcatAttrs nlmsg.BytesView, keyLen uint32) (concat []uint8, err *syserr.AnnotatedError) {
	...
	for !concatAttrsList.Empty() {
		if i >= len(concatList) {
			return nil, syserr.NewAnnotatedError(syserr.ErrFileTooBig, "Nftables: Too many set description concat elements")
		}
		hdr, value, rest, ok := concatAttrsList.ParseFirst()
		if !ok {
			return nil, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Failed to parse set description concat element")
		}
		concatAttrsList = rest
		if hdr.Type != linux.NFTA_LIST_ELEM {
			return nil, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Set description concat element is not of type NFTA_LIST_ELEM")
		}
		concatElem, ok := NfParseWithOpts(nlmsg.AttrsView(value), &NfParseOpts{
			Policy: setConcatPolicy,
		})
		if !ok {
			return nil, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Failed to parse set description concat element attributes")
		}
		fieldLen, ok := AttrNetToHost[uint32](linux.NFTA_SET_FIELD_LEN, concatElem)
		if !ok {
			return nil, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Failed to parse set description field length")
		}
		if fieldLen == 0 || fieldLen > math.MaxUint8 {
			return nil, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Invalid set field length")
		}
	...
	if totalLen != keyLen {
		return nil, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Total set description field length does not match key length")
	}
	if keyLen > linux.NFT_REG32_COUNT {
		return nil, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Key length is larger than maximum supported register size")
	}
...
func parseSetDescAttr(attrs map[uint16]nlmsg.BytesView, keyLen uint32, setFlags uint16) (concat []uint8, descSize uint32, err *syserr.AnnotatedError) {
	...
	if !ok {
		if setFlags&linux.NFT_SET_CONCAT != 0 {
			return nil, 0, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Set concat flag present but set description attribute is missing")
		}
		return nil, 0, nil
	}
	...
	if !ok {
		return nil, 0, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Failed to parse set description attributes")
	}
	...
	if len(concat) > 1 {
		if setFlags&linux.NFT_SET_CONCAT == 0 {
			return nil, 0, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Set concat flag missing but multiple concat elements found")
		}
	} else if setFlags&linux.NFT_SET_CONCAT != 0 {
		return nil, 0, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Set concat flag present but single/no concat elements found")
	}
...
func (nf *NFTables) NewSet(attrs map[uint16]nlmsg.BytesView, family stack.AddressFamily, msgFlags uint16, ms *nlmsg.MessageSet) *syserr.AnnotatedError {
	...
	if !tabNameOk {
		return syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Set table name attribute is malformed or not found")
	}

	setNameBytes, nameOk := attrs[linux.NFTA_SET_NAME]
	if !nameOk {
		return syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Set name attribute is malformed or not found")
	}

	if _, idOk := attrs[linux.NFTA_SET_ID]; !idOk {
		return syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Set id attribute is missing")
	}
	...
	if !keyLenOk {
		return syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Set key length attribute is missing")
	}
	if keyLen == 0 || keyLen > linux.NFT_DATA_VALUE_MAXLEN {
		return syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Set key length attribute is invalid")
	}
	...
	if uint32(msgFlags)&linux.NLM_F_CREATE == 0 {
		return syserr.NewAnnotatedError(syserr.ErrNotSupported, "Nftables: NLM_F_CREATE is required for set creation")
	}
	...
	if set, exists := tab.sets[setName]; exists {
		if msgFlags&linux.NLM_F_EXCL != 0 {
			return syserr.NewAnnotatedError(syserr.ErrExists, "Nftables: Set already exists")
		}
		if msgFlags&linux.NLM_F_REPLACE != 0 {
			return syserr.NewAnnotatedError(syserr.ErrNotSupported, "Nftables: Replace flag is not supported")
		}
		if set.flags&linux.NFT_SET_ANONYMOUS != 0 {
			return syserr.NewAnnotatedError(syserr.ErrNotSupported, "Nftables: Anonymous set already exists")
		}
	...
	if setFlags&linux.NFT_SET_MAP == 0 {
		return syserr.NewAnnotatedError(syserr.ErrNotSupported, "Nftables: only map sets are supported yet")
	}

	newSet.backend = newSetMapBackend(int(keyLen), int(dataLen))
...
func (nf *NFTables) validateElemFlags(setFlags uint16, elemFlags uint16) *syserr.AnnotatedError {
	if elemFlags & ^(linux.NFT_SET_ELEM_INTERVAL_END|linux.NFT_SET_ELEM_CATCHALL) != 0 {
		return syserr.NewAnnotatedError(syserr.ErrNotSupported, "Nftables: Unsupported set element flags")
	}
	if (setFlags&linux.NFT_SET_INTERVAL) == 0 &&
		(elemFlags&linux.NFT_SET_ELEM_INTERVAL_END != 0) {
		return syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Interval end flag is only supported for interval sets")
	}
	if (elemFlags & (linux.NFT_SET_ELEM_INTERVAL_END | linux.NFT_SET_ELEM_CATCHALL)) ==
		...
		return syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Interval end and catchall flags cannot be set at the same time")
	}
	return nil
}
...
func (nf *NFTables) parseElemDataAttr(tab *Table, set *nftSet, dataAttrs nlmsg.BytesView) (*dataOrVerdict, *syserr.AnnotatedError) {
	...
	if !ok {
		return nil, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Failed to parse set element data attribute")
	}

	var dataValue []byte
	var verdict stack.NFVerdict
	dataType := linux.NFT_DATA_VALUE
	if dv, err := parseDataAttrs(dataValueAttr); err == nil {
		dataValue = dv
	...
	if dataType == linux.NFT_DATA_VALUE {
		lenData := len(dataValue)
		if lenData != int(set.dataLen) {
			return nil, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Data length does not match set data length")
		}
	} else if dataType == linux.NFT_DATA_VERDICT {
...
func (e *nftSetElem) conflicts(other *nftSetElem) bool {
	...
	// TODO: b/434243967 - Compare objref when supported.
	return false
}
...
func (s *nftSet) addCatchAllElement(elem *nftSetElem, msgFlags uint16) *syserr.AnnotatedError {
	...
	if msgFlags&linux.NLM_F_EXCL != 0 {
		return syserr.NewAnnotatedError(syserr.ErrExists, "Nftables: catchall element already exists")
	}
	if !catchAllElem.conflicts(elem) {
		return nil
	}
	return syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: catchall element already exists and is not the same")
}
...
func (s *nftSet) commitElement(elem *nftSetElem, msgFlags uint16) *syserr.AnnotatedError {
	...
	return syserr.NewAnnotatedError(syserr.ErrExists, "Nftables: Set element already exists")
}
...
func (nf *NFTables) addElemToSet(tab *Table, set *nftSet, elemAttrs map[uint16]nlmsg.BytesView, msgFlags uint16) *syserr.AnnotatedError {
	...
	if set.descSize > 0 && len(set.elements) >= int(set.descSize) {
		return syserr.NewAnnotatedError(syserr.ErrTooManyOpenFiles, "Nftables: Set element limit reached")
	}
	...
	if keyExists && (flag&linux.NFT_SET_ELEM_CATCHALL != 0) {
		return syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Set element key attribute should not be present for catchall element")
	}
	if !keyExists && (flag&linux.NFT_SET_ELEM_CATCHALL == 0) {
		return syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Set element key attribute is missing for non-catchall element")
	}
	...
	if (set.flags & linux.NFT_SET_MAP) != 0 {
		if !dataExists && (flag&linux.NFT_SET_ELEM_INTERVAL_END == 0) {
			return syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Set element data attribute is missing for non-interval element")
		}
	} else if dataExists {
		return syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Set element data attribute should not be present for non-map set")
	}
	...
	if (set.flags & linux.NFT_SET_OBJECT) != 0 {
		if !objRefExists && (flag&linux.NFT_SET_ELEM_INTERVAL_END == 0) {
			return syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Set element object reference attribute is missing for non-interval element")
		}
	} else if objRefExists {
		return syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Set element object reference attribute should not be present for non-object set")
	}
	...
	if (flag&linux.NFT_SET_ELEM_INTERVAL_END) != 0 &&
		...
		return syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Interval end flag is set but interval end attributes are missing")
	}
	...
	if ok {
		if (set.flags & linux.NFT_SET_TIMEOUT) == 0 {
			return syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Timeout flag is not set but timeout attribute is present")
		}
	...
	if ok {
		if (set.flags & linux.NFT_SET_TIMEOUT) == 0 {
			return syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Expiration flag is not set but expiration attribute is present")
		}
		if timeout == 0 {
			return syserr.NewAnnotatedError(syserr.ErrNotSupported, "Nftables: Expiration attribute present but timeout is zero")
		}
		if expiration > timeout {
			return syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Expiration time is greater than timeout")
		}
	}
	...
	if ok {
		if len(set.exprInfos) > 0 && len(set.exprInfos) != 1 {
			return syserr.NewAnnotatedError(syserr.ErrNotSupported, "Nftables: Only one expression is supported for set element expression")
		}
	...
	if len(exprInfos) != 0 {
		if len(set.exprInfos) > 0 {
			if len(exprInfos) != len(set.exprInfos) {
				return syserr.NewAnnotatedError(syserr.ErrNotSupported, "Nftables: Number of set element expressions does not match number of set operations")
			}
			for i, exprInfo := range exprInfos {
				if exprInfo.ExprName != set.exprInfos[i].ExprName {
					return syserr.NewAnnotatedError(syserr.ErrNotSupported, "Nftables: Set element expression operation does not match set operation")
				}
			}
		}
	...
	// Convert expressions to operations
	// to execute at evaluation time.
	...
	if objRefExists {
		// TODO: b/434243967 - Support object reference attribute.
		return syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Object reference attribute is not supported")
	}
...
// setMapBackendKey is the key type for the set map backend.
// Fixed size array can be directly used as map key.
type setMapBackendKey [linux.NFT_DATA_VALUE_MAXLEN]byte

// setMapBackend is a backend to handle map operations.
type setMapBackend struct {
	keyLen  int
	dataLen int
	m       map[setMapBackendKey]int
}

// Lookup implements NftSetBackend.Lookup.
func (s *setMapBackend) Evaluate(regs *registerSet, keyIdx int) int {
	var key setMapBackendKey
	copy(key[:], regs.data[keyIdx:keyIdx+s.keyLen])
	v, ok := s.m[key]
	if !ok {
		return -1
	}
...
func (s *setMapBackend) Add(e *nftSetElem, idx int) (int, *syserr.AnnotatedError) {
	...
	var key [linux.NFT_DATA_VALUE_MAXLEN]byte
	copy(key[:], e.startKey)
	if v, ok := s.m[key]; ok {
		return v, nil
	}
...
func (s *setMapBackend) Clone() NftSetBackend {
	mCopy := make(map[setMapBackendKey]int, len(s.m))
	for k, v := range s.m {
		mCopy[k] = v
	}
...
func newSetMapBackend(keyLen int, dataLen int) *setMapBackend {
	return &setMapBackend{
		keyLen:  keyLen,
		dataLen: dataLen,
		m:       make(map[setMapBackendKey]int),
	}
}
